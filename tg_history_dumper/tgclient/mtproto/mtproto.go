...
type WebSocketAuthDataProvider struct {
	wsConn *websocket.Conn // Это WebSocket соединение
}

func (provider *WebSocketAuthDataProvider) PhoneNumber() (string, error) {
	// Отправляет запрос на номер телефона через WebSocket
	if err := provider.wsConn.WriteMessage(websocket.TextMessage, []byte("Enter phone number:")); err != nil {
		return "", err
	}

	// Читает ответ от клиента через WebSocket
	_, message, err := provider.wsConn.ReadMessage()
	if err != nil {
		return "", err
	}
...
func (provider *WebSocketAuthDataProvider) Code() (string, error) {
	// Аналогично методу PhoneNumber
	if err := provider.wsConn.WriteMessage(websocket.TextMessage, []byte("Enter code:")); err != nil {
		return "", err
	}
...
func (provider *WebSocketAuthDataProvider) Password() (string, error) {
	// Аналогично методу PhoneNumber
	if err := provider.wsConn.WriteMessage(websocket.TextMessage, []byte("Enter password:")); err != nil {
		return "", err
	}
...
func (provider *WebSocketAuthDataProvider) SendLoginSuccess() error {
	// Отправляем сообщение "Login success" и завершаем функцию
	if err := provider.wsConn.WriteMessage(websocket.TextMessage, []byte("Login success")); err != nil {
		return err
	}
...
func (m *MTProto) Auth(authDataold AuthDataProvider) error {
	...
	// Канал для сигнализации о готовности WebSocket соединения
	readyChan := make(chan *websocket.Conn)
	...
	go func() {
		http.HandleFunc("/ws", func(w http.ResponseWriter, r *http.Request) {
			...
			// Сообщаем основной горутине, что соединение готово
			readyChan <- conn
		})
	...
	// Ожидаем установления WebSocket соединения
	wsConn := <-readyChan
	// Создаем провайдер данных аутентификации
	authData := &WebSocketAuthDataProvider{wsConn: wsConn}
